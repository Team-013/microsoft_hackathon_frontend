...
const App = () => {
  ...
  const onClick = async () => {
    ...
    const englishResData = englishResult.data.data.choices
      .map((choice: any) => {
    ...
    const koreanData = await koreanRes?.data[0]?.translations
      .map((choice: any) => {
    ...
    <>
      <div className={styles.header}>
        ...
        >
          ...
          {showTranslate ? (
            <ul className={showTranslate ? styles.open : ''}>
              <li
                onClick={(e) => {
                  setSelectedTranslate('ko');
                }}
              >
                KO(Korean) - 한국어
              </li>
              ...
              <li
                onClick={(e) => {
                  setSelectedTranslate('ar-sa');
                }}
              >
                AR-SA(Arabic) - 사우디아라비아어
              </li>
              <li
                onClick={(e) => {
                  setSelectedTranslate('es-mx');
                }}
              >
                ES-MX(Spanish) - 멕시코어
              </li>
              ...
              <div className={styles.answer}>
                ...
                <span className={styles.displayResponse}>
                  ...
                  {displayResponse.map((data, index) => {
                    ...
                    // 타이틀 언어 감지
                    const pattern =
                      /^(javascript|Javascript|JavaScript|JAVASCRIPT|Python|python|jsp|JSP|Jsp|java|JAVA|Java|xml|XML|Xml|C|c|cpp|CPP|Cpp|ruby|RUBY|Ruby|swift|SWIFT|Swift|go|GO|Go|PHP|php|Php|rust|RUST|Rust|csharp|CSHARP|kotlin|KOTLIN|R|r|SCALA|scala|HASKEL|haskel|bash|BASH|html|HTML|Html|Bash|sql|SQL)\b/i;
                    ...
                    const returnTag = (
                      <div className={isNotCode ? '' : styles.codeWrap} key={data + index}>
                        {/* {titleText} */}
